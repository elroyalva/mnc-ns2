...
static class ESMACClass : public TclClass {
public:
    ESMACClass() : TclClass("Mac/ESMAC") {}
...
// Added by Sushmita to support event tracing (singal@nunki.usc.edu).
void ESMAC::trace_event(char *eventtype, Packet *p)
{
    if (et_ == NULL) return;
    char *wrk = et_->buffer();
...
ESMAC::ESMAC() : Mac() {
    ...
    tx_active_ = 0;
    waitTimer = new ESMACWaitTimer(this);
    sendTimer = new ESMACSendTimer(this);
    recvTimer = new ESMACRecvTimer(this);
...
// Added by Sushmita to support event tracing (singal@nunki.usc.edu)
int
ESMAC::command(int argc, const char*const* argv)
{
    if (argc == 3) {
...
void ESMAC::recv(Packet *p, Handler *h) {
...
void ESMAC::send(Packet *p, Handler *h)
{
...
void ESMACTimer::restart(double time)
{
    if (busy_)
    stop();
    start(time);
...
void ESMACTimer::start(double time)
{
...
void ESMACWaitTimer::handle(Event *)
{
    busy_ = 0;
    stime = rtime = 0.0;
...
void ESMACSendTimer::handle(Event *)
{
    busy_ = 0;
    stime = rtime = 0.0;
...
void ESMACRecvTimer::handle(Event *)
{
    busy_ = 0;
    stime = rtime = 0.0;
