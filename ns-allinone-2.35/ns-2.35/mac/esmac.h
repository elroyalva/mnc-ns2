...
class ESMACWaitTimer;
class ESMACSendTimer;
class ESMACRecvTimer;

// Added by Sushmita to support event tracing (singal@nunki.usc.edu)
class EventTrace;


class ESMAC : public Mac {
...
private:
    ...
    Handler * 	txHandler_;
    ESMACWaitTimer *waitTimer;
    ESMACSendTimer *sendTimer;
    ESMACRecvTimer *recvTimer;
...
class ESMACTimer: public Handler {
public:
    ESMACTimer(ESMAC* m) : mac(m) {
        busy_ = 0;
    }
...
class ESMACWaitTimer: public ESMACTimer {
public: ESMACWaitTimer(ESMAC *m) : ESMACTimer(m) {}
    void handle(Event *e);
};

//  Timer to use for finishing sending of packets
class ESMACSendTimer: public ESMACTimer {
public:
    ESMACSendTimer(ESMAC *m) : ESMACTimer(m) {}
    void handle(Event *e);
};

// Timer to use for finishing reception of packets
class ESMACRecvTimer: public ESMACTimer {
public:
    ESMACRecvTimer(ESMAC *m) : ESMACTimer(m) {}
    void handle(Event *e);
};
